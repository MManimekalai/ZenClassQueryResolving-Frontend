      ...
      fetchAdminTickets()
        .then((data) => {
          if (data && data.tickets) {
            ...
            const openTicketsfinal = openTickets.filter((ticket) => ticket.isAssigned !== true);
            setTicketData(openTicketsfinal);
          }
        })
        .catch((error) => {
        ...
        {ticketData.length === 0 ? ( // Check if there are no tickets assigned
          ...
          ticketData.map((ticket) => (
            <tr key={ticket.ticketNumber}>
              ...
              <td>{ticket.status}</td>
              <td>
                <Link to={`/admin/assign-ticket/${ticket.ticketNumber}`}>Assign Ticket</Link>
              </td>
            </tr>
            ))
            )}
