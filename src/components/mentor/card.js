...
import { fetchMentorDashboardData } from '../APIcalls/mentor';
...
function Card() {
  ...
  const generateCard = (cardInfo, index) => {
    ...
    const filteredTickets = cardInfo.filterStatus
      ? ticketData.filter((ticket) => ticket.Status === cardInfo.filterStatus) // Updated 'Status' to match your data structure
      : ticketData;
